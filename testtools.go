...
`)
	...
	// before /A)
	RegisterCleanupAction(90, fmt.Sprintf("zpool list; cat /proc/self/mountinfo; for MNT in `grep %s /proc/self/mountinfo | cut -f 5 -d ' ' | awk '{ print length, $0 }' | sort -nr | cut -d ' ' -f2-`; do umount -f $MNT; done",
		testDirName(stamp),
	))
